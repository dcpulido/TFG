...
public class AgentRenderer extends VertexRenderer implements CellViewRenderer, Serializable {
  ...
  static {
    ...
    try {
		...
		RenderComponentManager.loadRenderFile("Agent", index,"/ingenias/editor/rendererxml/AgentUMLPanel.xml");

		index=ViewPreferences.ViewType.INGENIAS;
		RenderComponentManager.loadRenderFile("Agent", index,"/ingenias/editor/rendererxml/AgentINGENIASPanel.xml");	
    }
    catch (Exception ex) {
		ex.printStackTrace();
    }
    ...
    return RenderComponentManager.getSize("Agent",current);
    }
	...
	public static void setEntity(Agent ent){
		Map currentMap=(Map)  RenderComponentManager.retrieveIDs("Agent",ent.getPrefs().getView());
	...
	{
	return RenderComponentManager.retrievePanel("Agent", ((Entity)((DefaultGraphCell)(view.getCell())).getUserObject()).getPrefs().getView());
	...
	public static JPanel setCurrent(ViewPreferences.ViewType c) {
		...
		return (JPanel) RenderComponentManager.retrievePanel("Agent",current);
	}
