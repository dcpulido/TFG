...
public class WPDependsEdge extends NAryEdge {
	
	// Constructor with data content
	public WPDependsEdge(WPDepends userObject) {
		super(userObject);
		
		this.addRole("WPDependssource");
		this.setArity("WPDependssource",true,1);
		this.setArity("WPDependssource",false,1);
		
		this.addClass("WPDependssource","ingenias.editor.entities.WorkProduct");
		this.addClass("WPDependssource","ingenias.editor.entities.BehaviourWP");
		this.addClass("WPDependssource","ingenias.editor.entities.StructuredWP");		
		this.addClass("WPDependssource","ingenias.editor.entities.StructuralWP");
		this.addClass("WPDependssource","ingenias.editor.entities.FreeWP");
		
		this.addRole("WPDependstarget");
		this.setArity("WPDependstarget",true,1);
		this.setArity("WPDependstarget",false,2147483647);
		
		this.addClass("WPDependstarget","ingenias.editor.entities.WorkProduct");
		this.addClass("WPDependstarget","ingenias.editor.entities.BehaviourWP");
		this.addClass("WPDependstarget","ingenias.editor.entities.StructuredWP");
		this.addClass("WPDependstarget","ingenias.editor.entities.StructuralWP");
		this.addClass("WPDependstarget","ingenias.editor.entities.FreeWP");
	...
	public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
		...
		if (nAryEdgesNum > 1 || edgesNum > 0 ||
			!((selectedEdge == null) || (selectedEdge instanceof WPDependsEdge)))
			return false;
		
		// n-edge to be considered.
		WPDependsEdge edge = new WPDependsEdge( null);
		...
		if (nAryEdgesNum == 1 && selectedEdge instanceof WPDependsEdge)
			edge = (WPDependsEdge) selectedEdge;
	...
	public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
		...
		for (int i = 0; i < selected.length; i++)
			if ( ! (selected[i] instanceof DefaultEdge ||
				...
				if (roles[i].equalsIgnoreCase("WPDependssource")){
					edges.add( new DefaultEdge( new WPDependssourceRole() ) );
				}
				
				if (roles[i].equalsIgnoreCase("WPDependstarget")){
					edges.add( new DefaultEdge( new WPDependstargetRole() ) );
				}
