...
public class WFContainsEdge extends NAryEdge {
	
	// Constructor with data content
	public WFContainsEdge(WFContains userObject) {
		super(userObject);
		
		this.addRole("WFContainssource");
		this.setArity("WFContainssource",true,1);
		this.setArity("WFContainssource",false,1);
		
		this.addClass("WFContainssource","ingenias.editor.entities.Activity");
		
		
		this.addRole("WFContainstarget");
		this.setArity("WFContainstarget",true,1);
		this.setArity("WFContainstarget",false,2147483647);
		
		this.addClass("WFContainstarget","ingenias.editor.entities.Activity");
		
		this.addClass("WFContainstarget","ingenias.editor.entities.TaskWS");
		
		this.addClass("WFContainstarget","ingenias.editor.entities.RoleWS");
		
		//this.addClass("WFContainstarget","ingenias.editor.entities.WorkProduct");
	...
	public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
		...
		if (nAryEdgesNum > 1 || edgesNum > 0 ||
			!((selectedEdge == null) || (selectedEdge instanceof WFContainsEdge)))
			return false;
		
		// n-edge to be considered.
		WFContainsEdge edge = new WFContainsEdge( null);
		...
		if (nAryEdgesNum == 1 && selectedEdge instanceof WFContainsEdge)
			edge = (WFContainsEdge) selectedEdge;
	...
	public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
		...
		for (int i = 0; i < selected.length; i++)
			if ( ! (selected[i] instanceof DefaultEdge ||
				...
				if (roles[i].equalsIgnoreCase("WFContainssource")){
					edges.add( new DefaultEdge( new WFContainssourceRole() ) );
				}
				
				if (roles[i].equalsIgnoreCase("WFContainstarget")){
					edges.add( new DefaultEdge( new WFContainstargetRole() ) );
				}
