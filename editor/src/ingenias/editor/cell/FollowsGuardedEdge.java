...
public class FollowsGuardedEdge extends NAryEdge {

  // Constructor with data content
  public FollowsGuardedEdge(FollowsGuarded userObject) {
    super(userObject);
    
     this.addRole("FollowsGuardedsource");
     this.setArity("FollowsGuardedsource",true,1);
     this.setArity("FollowsGuardedsource",false,1);
     
this.addClass("FollowsGuardedsource","ingenias.editor.entities.MentalState");
     
    
     this.addRole("FollowsGuardedtarget");
     this.setArity("FollowsGuardedtarget",true,1);
     this.setArity("FollowsGuardedtarget",false,2147483647);
     
this.addClass("FollowsGuardedtarget","ingenias.editor.entities.MentalEntity");
     
this.addClass("FollowsGuardedtarget","ingenias.editor.entities.MentalInstanceSpecification");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof FollowsGuardedEdge)))
      return false;

    // n-edge to be considered.
    FollowsGuardedEdge edge = new FollowsGuardedEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof FollowsGuardedEdge)
      edge = (FollowsGuardedEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
    ...
    if (roles[i].equalsIgnoreCase("FollowsGuardedsource")){
     edges.add( new DefaultEdge( new FollowsGuardedsourceRole() ) );
     }
      
    if (roles[i].equalsIgnoreCase("FollowsGuardedtarget")){
     edges.add( new DefaultEdge( new FollowsGuardedtargetRole() ) );
     }
