...
public class RefinesEdge extends NAryEdge {

  // Constructor with data content
  public RefinesEdge(Refines userObject) {
    super(userObject);
    
     this.addRole("Refinessource");
     this.setArity("Refinessource",true,1);
     this.setArity("Refinessource",false,1);
     
this.addClass("Refinessource","ingenias.editor.entities.Task");
     
this.addClass("Refinessource","ingenias.editor.entities.UseCase");
     
    
     this.addRole("Refinestarget");
     this.setArity("Refinestarget",true,1);
     this.setArity("Refinestarget",false,2147483647);
     
this.addClass("Refinestarget","ingenias.editor.entities.Task");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof RefinesEdge)))
      return false;

    // n-edge to be considered.
    RefinesEdge edge = new RefinesEdge( null);
    // If the connection involves an existing n-edge its assignations has to be considered.
    if (nAryEdgesNum == 1 && selectedEdge instanceof RefinesEdge)
      edge = (RefinesEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
	...
	if (roles[i].equalsIgnoreCase("Refinessource")){
 	 edges.add( new DefaultEdge( new RefinessourceRole() ) );
	 }
      
	if (roles[i].equalsIgnoreCase("Refinestarget")){
 	 edges.add( new DefaultEdge( new RefinestargetRole() ) );
	 }
