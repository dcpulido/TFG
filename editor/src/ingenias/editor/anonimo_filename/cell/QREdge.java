...
public class QREdge extends NAryEdge {

  // Constructor with data content
  public QREdge(QR userObject) {
    super(userObject);
    
     this.addRole("QRsource");
     this.setArity("QRsource",true,1);
     this.setArity("QRsource",false,1);
     
this.addClass("QRsource","ingenias.editor.entities.MentalState");
     
    
     this.addRole("QRtarget");
     this.setArity("QRtarget",true,1);
     this.setArity("QRtarget",false,2147483647);
     
this.addClass("QRtarget","ingenias.editor.entities.MentalEntity");
     
this.addClass("QRtarget","ingenias.editor.entities.MentalInstanceSpecification");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof QREdge)))
      return false;

    // n-edge to be considered.
    QREdge edge = new QREdge( null);
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
    ...
    if (roles[i].equalsIgnoreCase("QRsource")){
     edges.add( new DefaultEdge( new QRsourceRole() ) );
     }
      
    if (roles[i].equalsIgnoreCase("QRtarget")){
     edges.add( new DefaultEdge( new QRtargetRole() ) );
     }
