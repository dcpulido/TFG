...
public class ActivityPrecedesEdge extends NAryEdge {
	
	// Constructor with data content
	public ActivityPrecedesEdge(ActivityPrecedes userObject) {
		super(userObject);
		
		this.addRole("ActivityPrecedessource");
		this.setArity("ActivityPrecedessource",true,1);
		this.setArity("ActivityPrecedessource",false,1);
		
		this.addClass("ActivityPrecedessource","ingenias.editor.entities.InitialNode");
		this.addClass("ActivityPrecedessource","ingenias.editor.entities.Phase");
		this.addClass("ActivityPrecedessource","ingenias.editor.entities.iPhase");
		this.addClass("ActivityPrecedessource","ingenias.editor.entities.Activity");
		this.addClass("ActivityPrecedessource","ingenias.editor.entities.ForkNode");
		this.addClass("ActivityPrecedessource","ingenias.editor.entities.JoinNode");
		this.addClass("ActivityPrecedessource","ingenias.editor.entities.DecisionNode");
		
		this.addRole("ActivityPrecedestarget");
		this.setArity("ActivityPrecedestarget",true,1);
		this.setArity("ActivityPrecedestarget",false,2147483647);
		
		this.addClass("ActivityPrecedestarget","ingenias.editor.entities.EndNode");
		this.addClass("ActivityPrecedestarget","ingenias.editor.entities.Phase");
		this.addClass("ActivityPrecedestarget","ingenias.editor.entities.iPhase");
		this.addClass("ActivityPrecedestarget","ingenias.editor.entities.Activity");
		this.addClass("ActivityPrecedestarget","ingenias.editor.entities.JoinNode");
		this.addClass("ActivityPrecedestarget","ingenias.editor.entities.DecisionNode");
		this.addClass("ActivityPrecedestarget","ingenias.editor.entities.ForkNode");
	...
	public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
		...
		if (nAryEdgesNum > 1 || edgesNum > 0 ||
			!((selectedEdge == null) || (selectedEdge instanceof ActivityPrecedesEdge)))
			return false;
		
		// n-edge to be considered.
		ActivityPrecedesEdge edge = new ActivityPrecedesEdge( null);
		...
		if (nAryEdgesNum == 1 && selectedEdge instanceof ActivityPrecedesEdge)
			edge = (ActivityPrecedesEdge) selectedEdge;
	...
	public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
		...
		for (int i = 0; i < selected.length; i++)
			if ( ! (selected[i] instanceof DefaultEdge ||
				...
				if (roles[i].equalsIgnoreCase("ActivityPrecedessource")){
					edges.add( new DefaultEdge( new ActivityPrecedessourceRole() ) );
				}
				
				if (roles[i].equalsIgnoreCase("ActivityPrecedestarget")){
					edges.add( new DefaultEdge( new ActivityPrecedestargetRole() ) );
				}
