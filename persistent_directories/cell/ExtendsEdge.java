...
public class ExtendsEdge extends NAryEdge {
    
    // Constructor with data content
    public ExtendsEdge(Extends userObject) {
        super(userObject);
        
        this.addRole("Extendssource");
        this.setArity("Extendssource",true,1);
        this.setArity("Extendssource",false,1);
        
        this.addClass("Extendssource","ingenias.editor.entities.UseCase");
        this.addClass("Extendssource","ingenias.editor.entities.DPDFSMMat");
        
        
        this.addRole("Extendstarget");
        this.setArity("Extendstarget",true,1);
        this.setArity("Extendstarget",false,1);
        
        this.addClass("Extendstarget","ingenias.editor.entities.UseCase");
        this.addClass("Extendstarget","ingenias.editor.entities.DPDFSMMel");
    ...
    public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
        ...
        if (nAryEdgesNum > 1 || edgesNum > 0 ||
            !((selectedEdge == null) || (selectedEdge instanceof ExtendsEdge)))
            return false;
        
        // n-edge to be considered.
        ExtendsEdge edge = new ExtendsEdge( null);
    ...
    public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
        ...
        for (int i = 0; i < selected.length; i++)
            if ( ! (selected[i] instanceof DefaultEdge ||
                ...
                if (roles[i].equalsIgnoreCase("Extendssource")){
                    edges.add( new DefaultEdge( new ExtendssourceRole() ) );
                }
                
                if (roles[i].equalsIgnoreCase("Extendstarget")){
                    edges.add( new DefaultEdge( new ExtendstargetRole() ) );
                }
