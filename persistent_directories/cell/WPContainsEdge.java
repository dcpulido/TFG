...
public class WPContainsEdge extends NAryEdge {
	
	// Constructor with data content
	public WPContainsEdge(WPContains userObject) {
		super(userObject);
		
		this.addRole("WPContainssource");
		this.setArity("WPContainssource",true,1);
		this.setArity("WPContainssource",false,1);
		
		this.addClass("WPContainssource","ingenias.editor.entities.WorkProduct");
		this.addClass("WPContainssource","ingenias.editor.entities.BehaviourWP");
		this.addClass("WPContainssource","ingenias.editor.entities.StructuredWP");		
		this.addClass("WPContainssource","ingenias.editor.entities.StructuralWP");
		this.addClass("WPContainssource","ingenias.editor.entities.FreeWP");
		
		this.addRole("WPContainstarget");
		this.setArity("WPContainstarget",true,1);
		this.setArity("WPContainstarget",false,2147483647);
		
		this.addClass("WPContainstarget","ingenias.editor.entities.WorkProduct");
		this.addClass("WPContainstarget","ingenias.editor.entities.BehaviourWP");
		this.addClass("WPContainstarget","ingenias.editor.entities.StructuredWP");
		this.addClass("WPContainstarget","ingenias.editor.entities.StructuralWP");
		this.addClass("WPContainstarget","ingenias.editor.entities.FreeWP");
		this.addClass("WPContainstarget","ingenias.editor.entities.DPDFSMMop");
		this.addClass("WPContainstarget","ingenias.editor.entities.DPDFSMMel");
		this.addClass("WPContainstarget","ingenias.editor.entities.DPDFSMMat");
	...
	public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
		...
		if (nAryEdgesNum > 1 || edgesNum > 0 ||
			!((selectedEdge == null) || (selectedEdge instanceof WPContainsEdge)))
			return false;
		
		// n-edge to be considered.
		WPContainsEdge edge = new WPContainsEdge( null);
		...
		if (nAryEdgesNum == 1 && selectedEdge instanceof WPContainsEdge)
			edge = (WPContainsEdge) selectedEdge;
	...
	public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
		...
		for (int i = 0; i < selected.length; i++)
			if ( ! (selected[i] instanceof DefaultEdge ||
				...
				if (roles[i].equalsIgnoreCase("WPContainssource")){
					edges.add( new DefaultEdge( new WPContainssourceRole() ) );
				}
				
				if (roles[i].equalsIgnoreCase("WPContainstarget")){
					edges.add( new DefaultEdge( new WPContainstargetRole() ) );
				}
