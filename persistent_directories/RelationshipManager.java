...
public class RelationshipManager implements java.io.Serializable {
	...
	private static Hashtable edgesAttributes(DefaultEdge[] edges,
		...
		for (int i = 0; i < roleAssignation.length; i++) {
			...
			// General
			if (roleAssignation[i].endsWith("ActivityPrecedessource")||
				roleAssignation[i].startsWith("WPDepends")            ){
            ...
            */
			...
			// Source
			if (roleAssignation[i].indexOf("source") >= 0){
				if (roleAssignation[i].startsWith("WFContains") ||
					roleAssignation[i].startsWith("WPContains")   ){
				...
				if (roleAssignation[i].startsWith("ActivityPrecedes")||
					roleAssignation[i].startsWith("WPDepends")      ){
					GraphConstants.setDashPattern(attr, new float[] { 6, 6 });
				}
			...
			// Target
			if (roleAssignation[i].indexOf("target") >= 0 ||
				...
				if (roleAssignation[i].startsWith("DPDFPrecedes")){
					GraphConstants.setLineEnd(attr, GraphConstants.ARROW_TECHNICAL);				
				...
				if (roleAssignation[i].startsWith("Extends")){
					GraphConstants.setLineEnd(attr, GraphConstants.ARROW_TECHNICAL);
					GraphConstants.setEndFill(attr, false);
				}
				
				if (roleAssignation[i].startsWith("UMLAssociation")){
					GraphConstants.setLineEnd(attr, GraphConstants.ARROW_NONE);
				}
		...
		if (ops.length > 0) {
			...
			if (nEdge != null) {
				...
				if (currentAssignation != null) {
					...
					RenderComponentManager.setRelationshipView(entNEdge.getPrefs().getView(),entNEdge,nEdge,graph);
