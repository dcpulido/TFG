...
implements java.io.Serializable {
	...
	public GeneralEditionPanel(Editor ed, Frame f, 
							   ...
							   ingenias.editor.GraphManager gm,
							   Entity ent) {
	...
	private Entity createEntity(Class entType) {
		...
		try {
			...
			Object[] paracf = {
				"" + ingenias.editor.Editor.getNewId(om,gm)};
	...
	ClassNotFoundException, IllegalAccessException, InstantiationException {
		...
		/*		jt.addFocusListener(
		 ...
		 JTextComponent jt = (JTextComponent) fe.getComponent();
	...
	private JPanel createModelPanel(ModelEntity me) {
		...
		top.add(value);
		//Middle add menu elements for some kind of entities like language in the CodeComponent element
	...
	InstantiationException, IllegalAccessException, InvocationTargetException {
		...
		if (ingenias.editor.entities.ModelEntity.class.isAssignableFrom(cf.getType())) {
			...
			if (this.getValue(this.entity.getClass(), cf) == null) {
				...
				Entity subentity = (Entity)this.getValue(this.entity.getClass(), cf);				
	...
	private void setValue(final Object value, final Field cf) {
		try {
			...
			if (!value.equals(oldvalue)){			
				if ((cf.getName().equalsIgnoreCase("id") && 
					 this.om.findUserObject(value.toString()).size()==0) 
					|| !cf.getName().equalsIgnoreCase("id")){		
					this.confirm.add(new ActionListener(){
						public void actionPerformed(ActionEvent e) {
							try {   
	...
	private void removeValue(final String id, Field cf) {
		try {
			 ...
			 *  }
			 */
			final java.lang.reflect.Method m = this.entity.getClass().getMethod(mname,
																				params);
			...
			// No need of undo here, since the undo consists in leaving it as it is now and not executing the remove action
