...
public class relatesEdge extends NAryEdge {

  // Constructor with data content
  public relatesEdge(relates userObject) {
    super(userObject);
    
     this.addRole("relatessource");
     this.setArity("relatessource",true,1);
     this.setArity("relatessource",false,1);
     
this.addClass("relatessource","ingenias.editor.entities.MentalState");
     
    
     this.addRole("relatestarget");
     this.setArity("relatestarget",true,1);
     this.setArity("relatestarget",false,2147483647);
     
this.addClass("relatestarget","ingenias.editor.entities.MentalEntity");
     
this.addClass("relatestarget","ingenias.editor.entities.MentalInstanceSpecification");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof relatesEdge)))
      return false;

    // n-edge to be considered.
    relatesEdge edge = new relatesEdge( null);
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
    ...
    if (roles[i].equalsIgnoreCase("relatessource")){
     edges.add( new DefaultEdge( new relatessourceRole() ) );
     }
      
    if (roles[i].equalsIgnoreCase("relatestarget")){
     edges.add( new DefaultEdge( new relatestargetRole() ) );
     }
