...
public class quotesEdge extends NAryEdge {

  // Constructor with data content
  public quotesEdge(quotes userObject) {
    super(userObject);
    
     this.addRole("quotessource");
     this.setArity("quotessource",true,1);
     this.setArity("quotessource",false,1);
     
this.addClass("quotessource","ingenias.editor.entities.MentalState");
     
    
     this.addRole("quotestarget");
     this.setArity("quotestarget",true,1);
     this.setArity("quotestarget",false,2147483647);
     
this.addClass("quotestarget","ingenias.editor.entities.MentalEntity");
     
this.addClass("quotestarget","ingenias.editor.entities.MentalInstanceSpecification");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof quotesEdge)))
      return false;

    // n-edge to be considered.
    quotesEdge edge = new quotesEdge( null);
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
    ...
    if (roles[i].equalsIgnoreCase("quotessource")){
     edges.add( new DefaultEdge( new quotessourceRole() ) );
     }
      
    if (roles[i].equalsIgnoreCase("quotestarget")){
     edges.add( new DefaultEdge( new quotestargetRole() ) );
     }
