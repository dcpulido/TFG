...
public class noneEdge extends NAryEdge {

  // Constructor with data content
  public noneEdge(none userObject) {
    super(userObject);
    
     this.addRole("nonesource");
     this.setArity("nonesource",true,1);
     this.setArity("nonesource",false,1);
     
this.addClass("nonesource","ingenias.editor.entities.MentalState");
     
    
     this.addRole("nonetarget");
     this.setArity("nonetarget",true,1);
     this.setArity("nonetarget",false,2147483647);
     
this.addClass("nonetarget","ingenias.editor.entities.MentalEntity");
     
this.addClass("nonetarget","ingenias.editor.entities.MentalInstanceSpecification");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof noneEdge)))
      return false;

    // n-edge to be considered.
    noneEdge edge = new noneEdge( null);
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
    ...
    if (roles[i].equalsIgnoreCase("nonesource")){
     edges.add( new DefaultEdge( new nonesourceRole() ) );
     }
      
    if (roles[i].equalsIgnoreCase("nonetarget")){
     edges.add( new DefaultEdge( new nonetargetRole() ) );
     }
