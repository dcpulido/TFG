...
public class FollowsEdge extends NAryEdge {

  // Constructor with data content
  public FollowsEdge(Follows userObject) {
    super(userObject);
    
     this.addRole("Followssource");
     this.setArity("Followssource",true,1);
     this.setArity("Followssource",false,1);
     
this.addClass("Followssource","ingenias.editor.entities.MentalState");
     
    
     this.addRole("Followstarget");
     this.setArity("Followstarget",true,1);
     this.setArity("Followstarget",false,2147483647);
     
this.addClass("Followstarget","ingenias.editor.entities.MentalEntity");
     
this.addClass("Followstarget","ingenias.editor.entities.MentalInstanceSpecification");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof FollowsEdge)))
      return false;

    // n-edge to be considered.
    FollowsEdge edge = new FollowsEdge( null);
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
    ...
    if (roles[i].equalsIgnoreCase("Followssource")){
     edges.add( new DefaultEdge( new FollowssourceRole() ) );
     }
      
    if (roles[i].equalsIgnoreCase("Followstarget")){
     edges.add( new DefaultEdge( new FollowstargetRole() ) );
     }
