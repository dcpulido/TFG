...
public class QQEdge extends NAryEdge {

  // Constructor with data content
  public QQEdge(QQ userObject) {
    super(userObject);
    
     this.addRole("QQsource");
     this.setArity("QQsource",true,1);
     this.setArity("QQsource",false,1);
     
this.addClass("QQsource","ingenias.editor.entities.MentalState");
     
    
     this.addRole("QQtarget");
     this.setArity("QQtarget",true,1);
     this.setArity("QQtarget",false,2147483647);
     
this.addClass("QQtarget","ingenias.editor.entities.MentalEntity");
     
this.addClass("QQtarget","ingenias.editor.entities.MentalInstanceSpecification");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof QQEdge)))
      return false;

    // n-edge to be considered.
    QQEdge edge = new QQEdge( null);
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
    ...
    if (roles[i].equalsIgnoreCase("QQsource")){
     edges.add( new DefaultEdge( new QQsourceRole() ) );
     }
      
    if (roles[i].equalsIgnoreCase("QQtarget")){
     edges.add( new DefaultEdge( new QQtargetRole() ) );
     }
