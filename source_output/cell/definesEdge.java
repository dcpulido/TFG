...
public class definesEdge extends NAryEdge {

  // Constructor with data content
  public definesEdge(defines userObject) {
    super(userObject);
    
     this.addRole("definessource");
     this.setArity("definessource",true,1);
     this.setArity("definessource",false,1);
     
this.addClass("definessource","ingenias.editor.entities.MentalState");
     
    
     this.addRole("definestarget");
     this.setArity("definestarget",true,1);
     this.setArity("definestarget",false,2147483647);
     
this.addClass("definestarget","ingenias.editor.entities.MentalEntity");
     
this.addClass("definestarget","ingenias.editor.entities.MentalInstanceSpecification");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof definesEdge)))
      return false;

    // n-edge to be considered.
    definesEdge edge = new definesEdge( null);
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
    ...
    if (roles[i].equalsIgnoreCase("definessource")){
     edges.add( new DefaultEdge( new definessourceRole() ) );
     }
      
    if (roles[i].equalsIgnoreCase("definestarget")){
     edges.add( new DefaultEdge( new definestargetRole() ) );
     }
