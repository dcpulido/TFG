...
public class QDEdge extends NAryEdge {

  // Constructor with data content
  public QDEdge(QD userObject) {
    super(userObject);
    
     this.addRole("QDsource");
     this.setArity("QDsource",true,1);
     this.setArity("QDsource",false,1);
     
this.addClass("QDsource","ingenias.editor.entities.MentalState");
     
    
     this.addRole("QDtarget");
     this.setArity("QDtarget",true,1);
     this.setArity("QDtarget",false,2147483647);
     
this.addClass("QDtarget","ingenias.editor.entities.MentalEntity");
     
this.addClass("QDtarget","ingenias.editor.entities.MentalInstanceSpecification");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof QDEdge)))
      return false;

    // n-edge to be considered.
    QDEdge edge = new QDEdge( null);
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
    ...
    if (roles[i].equalsIgnoreCase("QDsource")){
     edges.add( new DefaultEdge( new QDsourceRole() ) );
     }
      
    if (roles[i].equalsIgnoreCase("QDtarget")){
     edges.add( new DefaultEdge( new QDtargetRole() ) );
     }
