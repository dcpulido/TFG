...
public class RelateEdge extends NAryEdge {

  // Constructor with data content
  public RelateEdge(Relate userObject) {
    super(userObject);
    
     this.addRole("Relatesource");
     this.setArity("Relatesource",true,1);
     this.setArity("Relatesource",false,1);
     
this.addClass("Relatesource","ingenias.editor.entities.MentalState");
     
    
     this.addRole("Relatetarget");
     this.setArity("Relatetarget",true,1);
     this.setArity("Relatetarget",false,2147483647);
     
this.addClass("Relatetarget","ingenias.editor.entities.MentalEntity");
     
this.addClass("Relatetarget","ingenias.editor.entities.MentalInstanceSpecification");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof RelateEdge)))
      return false;

    // n-edge to be considered.
    RelateEdge edge = new RelateEdge( null);
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
    ...
    if (roles[i].equalsIgnoreCase("Relatesource")){
     edges.add( new DefaultEdge( new RelatesourceRole() ) );
     }
      
    if (roles[i].equalsIgnoreCase("Relatetarget")){
     edges.add( new DefaultEdge( new RelatetargetRole() ) );
     }
