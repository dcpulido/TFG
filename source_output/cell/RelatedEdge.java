...
public class RelatedEdge extends NAryEdge {

  // Constructor with data content
  public RelatedEdge(Related userObject) {
    super(userObject);
    
     this.addRole("Relatedsource");
     this.setArity("Relatedsource",true,1);
     this.setArity("Relatedsource",false,1);
     
this.addClass("Relatedsource","ingenias.editor.entities.MentalState");
     
    
     this.addRole("Relatedtarget");
     this.setArity("Relatedtarget",true,1);
     this.setArity("Relatedtarget",false,2147483647);
     
this.addClass("Relatedtarget","ingenias.editor.entities.MentalEntity");
     
this.addClass("Relatedtarget","ingenias.editor.entities.MentalInstanceSpecification");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof RelatedEdge)))
      return false;

    // n-edge to be considered.
    RelatedEdge edge = new RelatedEdge( null);
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
    ...
    if (roles[i].equalsIgnoreCase("Relatedsource")){
     edges.add( new DefaultEdge( new RelatedsourceRole() ) );
     }
      
    if (roles[i].equalsIgnoreCase("Relatedtarget")){
     edges.add( new DefaultEdge( new RelatedtargetRole() ) );
     }
