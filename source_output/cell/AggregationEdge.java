...
public class AggregationEdge extends NAryEdge {

  // Constructor with data content
  public AggregationEdge(Aggregation userObject) {
    super(userObject);
    
     this.addRole("Aggregationsource");
     this.setArity("Aggregationsource",true,1);
     this.setArity("Aggregationsource",false,1);
     
this.addClass("Aggregationsource","ingenias.editor.entities.MentalState");
     
    
     this.addRole("Aggregationtarget");
     this.setArity("Aggregationtarget",true,1);
     this.setArity("Aggregationtarget",false,2147483647);
     
this.addClass("Aggregationtarget","ingenias.editor.entities.MentalEntity");
     
this.addClass("Aggregationtarget","ingenias.editor.entities.MentalInstanceSpecification");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof AggregationEdge)))
      return false;

    // n-edge to be considered.
    AggregationEdge edge = new AggregationEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof AggregationEdge)
      edge = (AggregationEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
    ...
    if (roles[i].equalsIgnoreCase("Aggregationsource")){
     edges.add( new DefaultEdge( new AggregationsourceRole() ) );
     }
      
    if (roles[i].equalsIgnoreCase("Aggregationtarget")){
     edges.add( new DefaultEdge( new AggregationtargetRole() ) );
     }
