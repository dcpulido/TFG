...
public class DefineEdge extends NAryEdge {

  // Constructor with data content
  public DefineEdge(Define userObject) {
    super(userObject);
    
     this.addRole("Definesource");
     this.setArity("Definesource",true,1);
     this.setArity("Definesource",false,1);
     
this.addClass("Definesource","ingenias.editor.entities.MentalState");
     
    
     this.addRole("Definetarget");
     this.setArity("Definetarget",true,1);
     this.setArity("Definetarget",false,2147483647);
     
this.addClass("Definetarget","ingenias.editor.entities.MentalEntity");
     
this.addClass("Definetarget","ingenias.editor.entities.MentalInstanceSpecification");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof DefineEdge)))
      return false;

    // n-edge to be considered.
    DefineEdge edge = new DefineEdge( null);
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
    ...
    if (roles[i].equalsIgnoreCase("Definesource")){
     edges.add( new DefaultEdge( new DefinesourceRole() ) );
     }
      
    if (roles[i].equalsIgnoreCase("Definetarget")){
     edges.add( new DefaultEdge( new DefinetargetRole() ) );
     }
