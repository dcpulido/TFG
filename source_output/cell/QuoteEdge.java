...
public class QuoteEdge extends NAryEdge {

  // Constructor with data content
  public QuoteEdge(Quote userObject) {
    super(userObject);
    
     this.addRole("Quotesource");
     this.setArity("Quotesource",true,1);
     this.setArity("Quotesource",false,1);
     
this.addClass("Quotesource","ingenias.editor.entities.MentalState");
     
    
     this.addRole("Quotetarget");
     this.setArity("Quotetarget",true,1);
     this.setArity("Quotetarget",false,2147483647);
     
this.addClass("Quotetarget","ingenias.editor.entities.MentalEntity");
     
this.addClass("Quotetarget","ingenias.editor.entities.MentalInstanceSpecification");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof QuoteEdge)))
      return false;

    // n-edge to be considered.
    QuoteEdge edge = new QuoteEdge( null);
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
    ...
    if (roles[i].equalsIgnoreCase("Quotesource")){
     edges.add( new DefaultEdge( new QuotesourceRole() ) );
     }
      
    if (roles[i].equalsIgnoreCase("Quotetarget")){
     edges.add( new DefaultEdge( new QuotetargetRole() ) );
     }
