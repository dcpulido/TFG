...
public class QuoteRelationshipEdge extends NAryEdge {

  // Constructor with data content
  public QuoteRelationshipEdge(QuoteRelationship userObject) {
    super(userObject);
    
     this.addRole("QuoteRelationshipsource");
     this.setArity("QuoteRelationshipsource",true,1);
     this.setArity("QuoteRelationshipsource",false,1);
     
this.addClass("QuoteRelationshipsource","ingenias.editor.entities.MentalState");
     
    
     this.addRole("QuoteRelationshiptarget");
     this.setArity("QuoteRelationshiptarget",true,1);
     this.setArity("QuoteRelationshiptarget",false,2147483647);
     
this.addClass("QuoteRelationshiptarget","ingenias.editor.entities.MentalEntity");
     
this.addClass("QuoteRelationshiptarget","ingenias.editor.entities.MentalInstanceSpecification");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof QuoteRelationshipEdge)))
      return false;

    // n-edge to be considered.
    QuoteRelationshipEdge edge = new QuoteRelationshipEdge( null);
    // If the connection involves an existing n-edge its assignations has to be considered.
    if (nAryEdgesNum == 1 && selectedEdge instanceof QuoteRelationshipEdge)
      edge = (QuoteRelationshipEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
    ...
    if (roles[i].equalsIgnoreCase("QuoteRelationshipsource")){
     edges.add( new DefaultEdge( new QuoteRelationshipsourceRole() ) );
     }
      
    if (roles[i].equalsIgnoreCase("QuoteRelationshiptarget")){
     edges.add( new DefaultEdge( new QuoteRelationshiptargetRole() ) );
     }
