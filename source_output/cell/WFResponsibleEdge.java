...
public class WFResponsibleEdge extends NAryEdge {

  // Constructor with data content
  public WFResponsibleEdge(WFResponsible userObject) {
    super(userObject);
    
     this.addRole("WFResponsiblesource");
     this.setArity("WFResponsiblesource",true,1);
     this.setArity("WFResponsiblesource",false,1);
     
this.addClass("WFResponsiblesource","ingenias.editor.entities.MentalState");
     
    
     this.addRole("WFResponsibletarget");
     this.setArity("WFResponsibletarget",true,1);
     this.setArity("WFResponsibletarget",false,2147483647);
     
this.addClass("WFResponsibletarget","ingenias.editor.entities.MentalEntity");
     
this.addClass("WFResponsibletarget","ingenias.editor.entities.MentalInstanceSpecification");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof WFResponsibleEdge)))
      return false;

    // n-edge to be considered.
    WFResponsibleEdge edge = new WFResponsibleEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof WFResponsibleEdge)
      edge = (WFResponsibleEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
    ...
    if (roles[i].equalsIgnoreCase("WFResponsiblesource")){
     edges.add( new DefaultEdge( new WFResponsiblesourceRole() ) );
     }
      
    if (roles[i].equalsIgnoreCase("WFResponsibletarget")){
     edges.add( new DefaultEdge( new WFResponsibletargetRole() ) );
     }
