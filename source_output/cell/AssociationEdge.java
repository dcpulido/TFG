...
public class AssociationEdge extends NAryEdge {

  // Constructor with data content
  public AssociationEdge(Association userObject) {
    super(userObject);
    
     this.addRole("Associationsource");
     this.setArity("Associationsource",true,1);
     this.setArity("Associationsource",false,1);
     
this.addClass("Associationsource","ingenias.editor.entities.MentalState");
     
    
     this.addRole("Associationtarget");
     this.setArity("Associationtarget",true,1);
     this.setArity("Associationtarget",false,2147483647);
     
this.addClass("Associationtarget","ingenias.editor.entities.MentalEntity");
     
this.addClass("Associationtarget","ingenias.editor.entities.MentalInstanceSpecification");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof AssociationEdge)))
      return false;

    // n-edge to be considered.
    AssociationEdge edge = new AssociationEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof AssociationEdge)
      edge = (AssociationEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
    ...
    if (roles[i].equalsIgnoreCase("Associationsource")){
     edges.add( new DefaultEdge( new AssociationsourceRole() ) );
     }
      
    if (roles[i].equalsIgnoreCase("Associationtarget")){
     edges.add( new DefaultEdge( new AssociationtargetRole() ) );
     }
