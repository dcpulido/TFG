...
public class CompositionEdge extends NAryEdge {

  // Constructor with data content
  public CompositionEdge(Composition userObject) {
    super(userObject);
    
     this.addRole("Compositionsource");
     this.setArity("Compositionsource",true,1);
     this.setArity("Compositionsource",false,1);
     
this.addClass("Compositionsource","ingenias.editor.entities.MentalState");
     
    
     this.addRole("Compositiontarget");
     this.setArity("Compositiontarget",true,1);
     this.setArity("Compositiontarget",false,2147483647);
     
this.addClass("Compositiontarget","ingenias.editor.entities.MentalEntity");
     
this.addClass("Compositiontarget","ingenias.editor.entities.MentalInstanceSpecification");
  ...
  // Connects an Agent with a Goal or an Agent with a Goal and a Fact.
   public static boolean acceptConnection(GraphModel model, GraphCell[] selected) {
    ...
    if (nAryEdgesNum > 1 || edgesNum > 0 ||
      !((selectedEdge == null) || (selectedEdge instanceof CompositionEdge)))
      return false;

    // n-edge to be considered.
    CompositionEdge edge = new CompositionEdge( null);
    ...
    if (nAryEdgesNum == 1 && selectedEdge instanceof CompositionEdge)
      edge = (CompositionEdge) selectedEdge;
...
// Is a list of possible assignations (List) where each assignation
  ...
  public DefaultEdge[] connectionsEdges(GraphCell[] selected, String[] roles) {
    ...
    if (roles[i].equalsIgnoreCase("Compositionsource")){
     edges.add( new DefaultEdge( new CompositionsourceRole() ) );
     }
      
    if (roles[i].equalsIgnoreCase("Compositiontarget")){
     edges.add( new DefaultEdge( new CompositiontargetRole() ) );
     }
