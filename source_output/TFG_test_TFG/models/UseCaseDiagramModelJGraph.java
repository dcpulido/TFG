...
public class UseCaseDiagramModelJGraph extends ModelJGraph {
	...
	protected void creaToolBar() {
		if (toolbar==null){
			...
			/*Image img_TextUseCase =
			 ImageLoader.getImage("images/musecase.png");
			...
			ImageLoader.getImage("images/iusecase40.png");
			...
			ImageLoader.getImage("images/org40.gif");
	...
	public Object[] getPossibleRelationships(GraphCell[] selected) {
		...
		if (selected.length >= 2 && edgesNum == 0) {
			...
			if (nAryEdgesNum == 0) {
				...
				// N-ary relationships. Sometimes they can be also binary.
				if (RefinesEdge.acceptConnection(this.getModel(), selected)) {
					v.add("Refines");
				}
				...
				if (selectedEdge instanceof RefinesEdge &&
					(RefinesEdge.acceptConnection(this.getModel(), selected))) {
					v.add("Refines");
				}
	...
	public DefaultGraphCell getInstanciaNRelacion(String relacion,
		...
		if (nAryEdgesNum <= 1 && edgesNum == 0) {
			...
			if (relacion.equalsIgnoreCase("Refines")) {
				// ResponsibleNEdge already exists.
				...
				// There is no NAryEdges in selected.
					...
					return new RefinesEdge(new Refines(getMJGraph().getNewId()));
				}
			}
